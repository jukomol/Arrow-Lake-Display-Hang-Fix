# Fixing Display Hangs on Wake: Intel Arrow Lake + NVIDIA RTX PRO Blackwell Laptops (Ubuntu 24.04)

If your laptop has an **Intel Arrow Lake (or Meteor Lake) CPU with integrated graphics**, paired with an **NVIDIA discrete GPU**, running **Ubuntu 24.04** — and the screen goes black and the whole machine becomes unresponsive after a period of inactivity, screen lock, or suspend — you're very likely hitting the same open upstream kernel bug documented here.

This writeup covers how the problem was diagnosed, what actually causes it, the mitigation that worked, and a script that applies it automatically.

**Tested on:** Dell Pro Max 16 Plus — Intel Arrow Lake iGPU — NVIDIA RTX PRO 5000 Blackwell — Ubuntu 24.04 — kernel `7.0.0-30-generic`.
Also independently reported on an HP ZBook Fury G1i with a different NVIDIA RTX PRO Blackwell model (see [References](#references)) — so this isn't specific to one OEM.

---

## TL;DR

1. Run [`fix-arrow-lake-display-hang.sh`](./fix-arrow-lake-display-hang.sh) with `sudo`.
2. Reboot.
3. Verify with `cat /proc/cmdline` — you should see `i915.enable_psr=0` and `i915.enable_dc=0`.
4. This won't fully eliminate the bug (it's still open upstream), but it converts a **hard hang requiring a forced power-cycle** into a **self-recovering ~1 minute stall**.
5. If it still hangs completely: a short press of the power button (which triggers suspend, then resume) has a good chance of un-wedging the display without a forced reboot.
...
- **The NVIDIA driver itself.** Confirmed in the upstream bug report (see References): `nvidia-suspend.service` and `nvidia-resume.service` complete cleanly every time. The NVIDIA GPU is not involved in the failure.
...
### Automated: `fix-arrow-lake-display-hang.sh`

The included script does all of the above, safely:
...
If you hit this on different hardware, please:
...
2. Consider opening a PR against this repo with your data point (hardware model, kernel version, whether the mitigation worked for you).
